...
The endpoint accepts `?memberID=...` and returns JSON with `authenticated`, `memberID`, and the matching `profile` data. The website saves theme changes with `?action=saveTheme&memberID=...&theme=dark` or `theme=light`.
